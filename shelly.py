    ...
    while True:
        ...
        if error:
            ...
            if result.error:
                ...
                vm_ = VM(create_instr('eggs_collected')+create_instr('sales')+create_instr('purchases')+[0, 'trades', 0, 1, 1, 0, 1, 1, 24, 25, 14, 26]+create_instr('trade')+create_instr('ds')+[0, 'dead_sick', 0, 1, 1, 0, 1, 1, 24, 25, 14, 26]+end[:-1]+[39, 31])
                cops = CommonOps()
                #vm_ = VM([37])
